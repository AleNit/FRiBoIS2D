...
!------------------------------------------ JW
         ...
         !transfer pressure from probes to marker
          dpdn=dot_product(lmacc(n,:),lmnor(n,:))
          press(n)=pc+dpdn*hpr              
         
         !friction stress: tau=2*nu*E; E=1/2*(gradu+gradu^T) 
          Ee(1,1)=ucy(2)
          Ee(1,2)=0.5*(ucy(3)+ucz(2))
          Ee(2,2)=ucz(3)
          Ee(2,1)=Ee(1,2) 

          tau(n,:)=2.0*visc*matmul(Ee,lmnor(n,:))  
...
!------------------------------------------ JW
     ...
     !------------------------------------------------------------
      ...
      real, intent(out) :: ucy(3), ucz(3), pc
     !------------------------------------------------------------
      integer :: n, i
      real :: pk, uk
     ...
     !------------------------------------------------------------
      ...
      ucy=0.0
      do n=1, dimSD
        uk=q2(eul(1,1,n),eul(1,2,n))
        do i=1, 3
          ucy(i)=ucy(i)+phi(1,i,n)*uk
        enddo
      enddo

      ucz=0.0
      do n=1, dimSD
        uk=q3(eul(2,1,n),eul(2,2,n))
        do i=1, 3
          ucz(i)=ucz(i)+phi(2,i,n)*uk
        enddo
      enddo

      pc=0.0
      do n=1, dimSD
        pk=pr(eul(3,1,n),eul(3,2,n))
        pc=pc+phi(3,1,n)*pk
      enddo
