...
function varargout=arrow3d(start,stop,ang,ltype,p,n,col)
%   Plot 3d Arrows
% 
...
if strcmp(ltype,'line')
    for i=1:length(dis)
        ...
        hlines(i)=line([start(i,1);start(i,1)+rx],[start(i,2);start(i,2)+ry],[start(i,3);start(i,3)+rz],'color',col);
    end
    hlgrd=[];
else
    for i=1:length(dis)
        ...
        hlines(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',col);
        hlgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),col);
    end
end
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',col);
    %Draw the underside of taper
    hhgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),col);
end
if SET_HOLD_OFF
    hold off;
end
