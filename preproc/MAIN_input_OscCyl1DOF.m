...
% Create input files for the oscillating cylinder test
...
%% input parameters
cname='test_OscCyl1DOF';        % case name 
wrt=false(1);                   % create input folder
l2e=60.0;                       % domain extension in the y direction
l3e=40.0;                       % domain extension in the z direction
nD=60;                          % number of Eulerian nodes over the cylinder diameter
z0=15+0.001;                    % z coordinate of the body rotation center
y0=ceil(l2e/2)+0.001;           % y coordinate of the body rotation center
rr=0.5;                         % lagrangian to eulerian grid spacing ratio 



%% generate cartesian grid
int3=[round(z0-2),round(z0+7)];
nint3=[ceil(nD/2),(int3(2)-int3(1))*nD,ceil(nD*4)+1];
...
%% create geometry
R=0.5;
nlm=ceil(2*pi*R/((dzmin+dymin)/2)/rr);
theta=linspace(0,360,nlm+1)'; 
zcp=R*cosd(theta)+z0;
ycp=R*sind(theta)+y0;
